...
function transferSuite(suite, totalTests, totalTime, status) {
  if (status === 'end') {
    totalTests.total += suite.tests.length;
  }
  ...
  suite.totalSkipped = suite.totalTests - passingTests.length - failingTests.length - pendingTests.length;
  suite.duration = duration;
  suite.tests = cleanTests;
}

function getSuite(suite, totalTests, status) {
  let totalTime = {
    time: 0
  };
  const queue = [];
  ...
  while (next) {
    ...
    if (next.root) {
      transferSuite(next, totalTests, totalTime, status);
    }

    if (next.suites && next.suites.length) {
      _.each(next.suites, (nextSuite, i) => {
        transferSuite(nextSuite, totalTests, totalTime, status);
