...
function done(output, config, failures, exit, options) {
  try {
...
function MacacaReporter(runner, options) {
  // const options = {fileName: 'fsdfds.html'}
  ...
  this.done = (failures, exit) => done(this.output, this.config, failures, exit, options);
  totalTests.total = 0;
